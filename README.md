...
<p>The goal of this project was to write a script 'poi_id.py' to predict an employee's person of interest("poi") label based on their financial and email features.  The poi label signified whether or not an employee was a person of interest in the Enron case.  Machine learning techniques were applied to data from 146 Enron employees.  There were 21 features for each employee and 3066 total data points.  The data for each employee fell into 3 categories: financial, email, and poi label.  There were more non-poi labels than poi, with 18 employees labeled as poi and 128 employees labeled as non-poi.  A variety of classification algorithms were deployed to predict an employee's poi label based on their financial and email features.  One algorithm was selected and tuned for final analysis.</p>

### Workflow

<p>'poi_id.py' opens the 'final_project_dataset.pkl' file and sorts the data into a dictionary of dictionaries.  The script 'helper.py' contains helper functions which were written to assist in outlier removal and feature creation.  The 'feature_format.py' file was imported to extract the features and labels from the dataset.  The classification algorithms were deployed and analyzed using metrics such as accuracy, precision, and recall.  The 'tester.py' file was imported to evaluate the performance of the final algorithm and write the pickle files for the classifier, dataset, and features.</p>
